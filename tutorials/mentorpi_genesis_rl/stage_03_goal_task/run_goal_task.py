...
from sim_mentor_pi.car_train import get_cfgs, update_policy_state_dim


def simple_goal_controller(obs_buf: torch.Tensor) -> torch.Tensor:
    target_x = obs_buf[:, 0]
    target_y = obs_buf[:, 1]
...
def main() -> None:
    ...
    obs_dict = env.reset()
    while True:
        actions = simple_goal_controller(obs_dict["policy"])
